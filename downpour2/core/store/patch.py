...
from storm.locals import StormError, Unicode
...
class Patch(object):
    ...
    version = Unicode(primary=True, allow_none=False)
...
class PatchApplier(object):
    ...
    def _name(self, version):
        return unicode('%s:%d' % (self._package.__name__, version))

    def _version(self, name):
        if name.startswith(self._package.__name__):
            return int(name[len(self._package.__name__) + 1:])
        return None
    ...
    def apply(self, version):
        ...
        """
        patch = Patch(self._name(version))
    ...
    def mark_applied(self, version):
        ...
        self._store.add(Patch(self._name(version)))
        self._committer.commit()
    ...
    def _get_applied_patches(self):
        ...
        for patch in self._store.find(Patch):
            num = self._version(patch.version)
            if not num is None:
                applied.add(num)
        return applied
